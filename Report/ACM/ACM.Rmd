...
```{r input-images, echo=FALSE, fig.align='center', fig.cap="**FIGURE 1.** Sample of training images.", fig.width=800, fig.height=500, message=FALSE, warning=FALSE, paged.print=FALSE}
library(knitr)
...
```{r pre-smote, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 2.** Pre-`SMOTE` truncated SVD.", fig.width=450, fig.height=450, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r post-smote, echo=FALSE, fig.align='center', fig.cap="**FIGURE 3.** Post-`SMOTE` truncated SVD.", fig.width=450, fig.height=450, message=FALSE, warning=FALSE, paged.print=FALSE}
...
The best choice of the optimizer for the `EKM` proved to be the `RMSprop`, while the standard `adam` performed pretty well with the `DenseNet121`. The manual implementations of the dropout and early stopping searches acted simultaneoulsy, so they performed like a grid search. The dropout, epoch values and weight corresponding to the highest mean column-wise ROC AUC were saved and used during the testing phase. 

Then, in order to establish the quantitative impact of stocasticity in the initialization of the weights on `EKM`, an `EKM1` with the best drop is trained and validated, and the best epochs of `EKM` and `EKM1` are compared. There was a small difference, therefore we decided to make three submissions: one with the baseline `EKM` re-trained on all the data and with the best drop, one with `EKM1` and one with the `DenseNet121`. 
...
```{r ae, echo=FALSE, fig.align='center', fig.cap='**FIGURE 4.** Schematic representation of the convolutional autoencoder.', fig.width=350, fig.height=500, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r basenet, echo=FALSE, fig.align='center', fig.cap='**FIGURE 5.** Schematic representation of the selected model architecture.', fig.width=350, fig.height=500, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r confusion-matrices, echo=FALSE, fig.align='center', fig.cap='**FIGURE 6.** Confusion matrices of `EKM` (left) and `DenseNet121` (right).', fig.width=800, fig.height=550, message=FALSE, warning=FALSE, paged.print=FALSE}
library(cowplot)
library(ggplot2)
...
```{r pca1, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 7.** The first ten principal directions of the PCA.", fig.width=800, fig.height=400, message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/PCA1.png")
```
```{r pca200, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 8.** Directions 200-210 of the PCA.",  fig.width=800, fig.height=400, message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/PCA200.png")
```
```{r variance, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 9.** Explained Variance.",  fig.width=550, fig.height=400, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r filters-ekm, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 13.** Three channels with some of the first 3x3 filters of the first hidden layer of the `EKM`.", fig.width=550, fig.height=460, message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/FiltersEKM.png")
```

```{r maps2, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 14.** Activation maps of the second hidden layer of `EKM`.", message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/FeatureMaps2.png")
```
```{r maps6, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 15.** Activation maps of the sixth hidden layer of `EKM`.", message=FALSE, warning=FALSE, paged.print=FALSE}
