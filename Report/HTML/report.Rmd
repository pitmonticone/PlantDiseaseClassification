...
Later in the report we will describe an attempt using a [convolutional autoencoder](#ae) while in the next section we can appreciate the amplification of the classes performed by `SMOTE` and recognize the clustering of the generated points.  
...
The best perfomance was obtained with baseline `SMOTE`, with some fine-tuning on the `sampling_strategy` ^[The value `all` means that all classes are resampled to match the size of the majority class.] and the `n_neighbors` parameters. For further details see [Platform Limitations](#platform-limitations).
...
We have adopted the Keras `ImageDataGenerator` and, after a manual inspection of the images, we found that the best data augmentation technique was a random planar rotation combined with random horizontal flip. For more details see the [Keras image pre-processing API](https://keras.io/api/preprocessing/image/).

#### Model Architecture Exploration

An extensve exploration of all models has been performed. here we report the *gridsearch* that ultimately proved to be better.
...
```{r ae, echo=FALSE, fig.align='center', fig.cap='**FIGURE 4.** Schematic representation of the convolutional autoencoder.', out.width=300, out.height=400, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r basenet, echo=FALSE, fig.align='center', fig.cap='**FIGURE 5.** Schematic representation of the selected model architecture.', out.width=300, out.height=400, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r confusion-matrices, echo=FALSE, fig.align='center', fig.cap='**FIGURE 6.** Confusion matrices of `EKM` (left) and `DenseNet121` (right).', message=FALSE, warning=FALSE, paged.print=FALSE}
library(cowplot)
library(ggplot2)
...
Although we have ultimately decided not to apply the PCA to reduce the dimensionality of the dataset, we believe it might be interesting to visualize the first ten principal directions and qualitatively compare them with a sequence of principal directions with lower retained variance.

```{r pca1, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 7.** The first ten principal directions of the PCA.", message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/PCA1.png")
```
```{r pca200, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 8.** Directions 200-210 of the PCA.", message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/PCA200.png")
```

As we could have reasonably expected the principal components with lower retained variance correspond to almost pure noise.

Here we plot the explained variance analysis and, using the criterion of 90% variance retention, we have obtained 429 components.

```{r variance, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 9.** Explained Variance.", message=FALSE, warning=FALSE, paged.print=FALSE}
include_graphics("Images/ExplainedVariance90.png")
```

#### *Filters and Activation Maps* {#filters-featuremaps}

**text / simple reference from main text.**

```{r filters-ekm, echo=FALSE, fig.align='center', fig.cap = "**FIGURE 10.** Three channels with some of the first 3x3 filters of the `EKM`.", message=FALSE, warning=FALSE, paged.print=FALSE}
